...
const ProductDetail = () => {
  ...
  const formatSize = (size) => {
    return size.replace("SIZE_", "Talle ");
  };
    ...
    <div className="container mx-auto p-6 bg-gray-100 rounded-lg mt-8 max-w-5xl">
      ...
      <div className="flex flex-col md:flex-row md:space-x-8">
        <div className="md:w-1/3 mb-4">
          <img
            src="https://nikearprod.vtexassets.com/arquivos/ids/1021645-1200-1200?width=1200&height=1200&aspect=true"
            alt={selectedProduct.model}
            className="w-full h-auto object-cover rounded-lg shadow-lg mb-4 cursor-pointer"
            onClick={() => handleImageClick("https://nikearprod.vtexassets.com/arquivos/ids/1021645-1200-1200?width=1200&height=1200&aspect=true")}
          />
        </div>
            ...
            >
              ...
              {Object.entries(sizeStockMap).map(([size, stock]) => (
                <option key={size} value={size} disabled={stock === 0}>
                  {formatSize(size)} {stock === 0 && "- Sin stock"}
                </option>
              ))}
            </select>
          </div>
      ...
      {showImageModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="relative bg-white p-4 rounded max-w-3xl mx-auto">
            <button
